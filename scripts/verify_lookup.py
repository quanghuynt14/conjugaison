...
# Les six d'origine, plus quatre que les mille verbes ont apportées : une forme
# qui n'existe qu'avec son pronom, un verbe impersonnel, deux graphies pour une
# même case, et l'auxiliaire, qui ne sort plus ses neuf cent cinquante tableaux.
DEFAULT_FORMS = ["vis", "fasse", "faites", "visse", "vît", "faire",
                 "souviens", "faut", "paye", "ai"]
...
def main():
    ...
    for form in sys.argv[1:] or DEFAULT_FORMS:
        ...
        # Mêmes tableaux que la page : les verbes qui empruntent la forme à
        # leur auxiliaire n'en sortent pas, ils sont comptés dans une phrase.
        groups, empruntes = B.split_groups(B.group_by_verb(expected.get(form, [])))
        if empruntes:
            print(f"        (auxiliaire de {len(empruntes)} autres verbes)")
        for verb, records in groups:
            rows = [f"{r.conjugated}{B.tense_label(r.slot)}" for r in records]
            block = "\n".join([B.lemma(verb), "".join(B.REVERSE_COLUMNS), *rows])
            whole = block in text
            if not whole:
                problems.append(
                    f"« {form} » : le tableau de « {B.lemma(verb)} » ne se "
            ...
            for record, row in zip(records, rows):
                ...
                print(f"        {mark} {B.lemma(verb):<14} "
                      f"{record.conjugated:<20} {B.tense_label(record.slot):<24} "
                      f"{record.accord or '—'}")
